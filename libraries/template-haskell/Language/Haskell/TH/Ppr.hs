...
pprForall ns ctxt
  -- even in the case without any ns, there could be a context (e.g.,
  -- in the case of pattern synonyms, where there are multiple forall
  -- binders and contexts).
  | [] <- ns  = pprCxt ctxt
  | otherwise = text "forall" <+> hsep (map ppr ns) <+> char '.' <+> pprCxt ctxt
