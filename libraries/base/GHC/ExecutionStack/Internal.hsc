...
-- | A pointer to the first stack frame of a 'StackTrace'.
firstChunk :: StackTrace -> Ptr Chunk
firstChunk (StackTrace fptr) =
...
stackFrames st@(StackTrace fptr) = iterChunk (firstChunk st)
  where
    ...
    -}
    iterChunk :: Ptr Chunk -> [Location]
    iterChunk ptr = iterFrames (chunkFrames chunk) (chunkFirstFrame chunk)
      where
        chunk = unsafePerformIO $ peekChunk ptr
        {-# NOINLINE chunk #-}

        iterFrames :: Word -> Ptr Addr -> [Location]
        iterFrames 0 _
          | chunkNext chunk == nullPtr = []
          | otherwise                  = iterChunk (chunkNext chunk)
        iterFrames n frame =
          ...
          where
            ...
            this = withForeignPtr fptr $ const $ do
                ...
                allocaBytes locationSize $ \buf -> do
                    libdw_my_cap_lookup_location pc buf
                    peekLocation buf

foreign import ccall unsafe "libdw_my_cap_get_backtrace"
    libdw_my_cap_get_backtrace :: IO (Ptr StackTrace)

foreign import ccall unsafe "libdw_my_cap_lookup_location"
    libdw_my_cap_lookup_location :: Addr -> Ptr Location -> IO ()
