...
newtype URing = URing (ForeignPtr URing)
...
instance Storable Sqe' where
    ...
    poke ptr sqe =
      ...
      where
        ...
        init :: Int32 -> Fd -> COff -> Ptr a -> Word64 -> IO ()
        init opcode fd offset addr len = do
            setOpCode opcode
            #{poke struct io_uring_sqe, flags} ptr 0
            #{poke struct io_uring_sqe, ioprio} ptr 0
            setFd fd
            #{poke struct io_uring_sqe, off} ptr offset
            #{poke struct io_uring_sqe, addr} ptr addr
            #{poke struct io_uring_sqe, len} ptr len
