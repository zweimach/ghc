...
pdataReprTyConExact tycon
  = do {   -- look up the representation tycon; if there is a match at all, it will be be exact
       ...
       ; (ptycon, _tys) <- pdataReprTyCon (tycon `mkTyConApp` mkTyCoVarTys (tyConTyCoVars tycon))
       ; return ptycon
       }
...
pdatasReprTyConExact tycon
  = do {   -- look up the representation tycon; if there is a match at all, it will be be exact
       ; (FamInstMatch { fim_instance = ptycon }) <- pdatasReprTyCon (tycon `mkTyConApp` mkTyCoVarTys (tyConTyCoVars tycon))
