...
getNameSpaceFastString :: OccName -> (NameSpace, FastString)
getNameSpaceFastString occ = (occNameSpace occ, occNameFS occ)

pattern OccName :: NameSpace -> FastString -> OccName
pattern OccName ns fs <- (getNameSpaceFastString -> (ns, fs))

-- To save space we combine a few things in the occFlags field,
--   * Namespace (encoded in bits 0-7)
--   * isDerivedOccName (encoded as bit 8)

occNameSpace :: OccName -> NameSpace
occNameSpace occ = toEnum $ occFlags occ .&. 0xff

occNameIsDerived :: OccName -> IsDerived
occNameIsDerived occ
  | testBit (occFlags occ) 8 = Derived
  | otherwise                = NotDerived

-- | Is an 'OccName' derived from another 'OccName'?
data IsDerived = Derived | NotDerived
               deriving (Eq, Ord)
...
mkOccNameFS' :: IsDerived -> NameSpace -> FastString -> OccName
mkOccNameFS' isDerived occ_sp str =
    OccName' (fromEnum occ_sp .|. derived) str
  where
    derived = case isDerived of Derived    -> bit 8
                                NotDerived -> 0

mkOccName :: NameSpace -> String -> OccName
mkOccName occ_sp str = mkOccNameFS' NotDerived occ_sp (mkFastString str)

mkOccNameFS :: NameSpace -> FastString -> OccName
mkOccNameFS occ_sp fs = mkOccNameFS' NotDerived occ_sp fs

mkDerivedOccNameFS :: NameSpace -> FastString -> OccName
mkDerivedOccNameFS occ_sp fs = mkOccNameFS' Derived occ_sp fs
...
demoteOccName occ = do
  space' <- demoteNameSpace (occNameSpace occ)
  return $ mkOccNameFS' (occNameIsDerived occ) space' (occNameFS occ)
...
setOccNameSpace sp occ = mkOccNameFS' (occNameIsDerived occ) sp (occNameFS occ)
...
mk_deriv :: NameSpace
         ...
         -> [FastString]    -- ^ The name we are deriving from in pieces which will
                            -- be concatenated
         -> OccName
mk_deriv occ_sp sys_prefix str = mkDerivedOccNameFS occ_sp (concatFS $ sys_prefix : str)
...
isDerivedOccName occ = occNameIsDerived occ == Derived
...
tidyOccName env occ@(OccName occ_sp fs)
  = case lookupUFM env fs of
      ...
      Just {} -> case lookupUFM env base1 of
                   Nothing -> (addToUFM env base1 2, mkOccNameFS' (occNameIsDerived occ) occ_sp base1)
                   Just n  -> find 1 n
  where
    ...
    find !k !n
      = case lookupUFM env new_fs of
          ...
          Nothing -> (new_env, mkOccNameFS' (occNameIsDerived occ) occ_sp new_fs)
       where
