...
mkEvBindsZonkEnv :: Bag EvBind -> ZonkEnv
mkEvBindsZonkEnv = mkZonkEnv . evBindsCvSubstEnv
...
zonkEvVarOcc env v
  | isCoVar v
  = EvCoercion . mkTcCoercion <$> zonkCoVarOcc env v
  | otherwise
...
zonkTcCoToCo env co
  ...
  where
    ...
    go (TcCoVarCo cv)         = mkTcCoercion <$> zonkCoVarOcc env cv
